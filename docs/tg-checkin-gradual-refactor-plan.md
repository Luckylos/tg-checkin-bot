...
- 当前建议下一步：**进入 Slice 3，拆分 `control.py` 的控制平面**
- 已完成：**Slice 2 config 域拆分已落地并通过 built image 验证**
  - 已拆出 `config_io.py` / `config_settings.py` / `config_accounts.py` / `config_jobs.py`
  - `config.py` 已收缩为稳定 facade，保留现有导入面
